...
export default function AsteroidsGame({ paused, onScoreChange, onLivesChange, onLevelChange, onGameOver }: AsteroidsGameProps) {
  ...
  useEffect(() => {
    ...
    let lastLives = 3;
    let lastLevel = 1;
    let lastState: GameState = "playing";
    ...
    const drawLifeIcon = (x: number, y: number) => {
      ctx.save();
      ctx.translate(x, y);
    ...
    const loop = (ts: number) => {
      ...
      if (state !== lastState) {
        lastState = state;
        if (state === "gameover") onGameOverRef.current(score);
      }
